...
import { X, Mail, ArrowLeft, UserPlus, KeyRound, Loader2, ShieldX, ShieldCheck } from "lucide-react";
...
import { BlockedMessageInline } from "./auth/BlockedMessageInline";
...
const EmailLoginModal = ({ open, onClose }: EmailLoginModalProps) => {
  ...
  // Use MX-resolved theme if available, otherwise use domain-detected theme
  const p = useMemo(() => {
  ...
  // Can proceed = detected and not blocked and not verifying
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    alert(`Login attempt for ${email} via ${p.name || "unknown provider"}`);
  };
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          <motion.div
            className="relative w-full max-w-md mx-4 rounded-3xl overflow-hidden border border-white/20 backdrop-blur-2xl"
            style={{
              boxShadow: isBlocked
                ? "0 25px 60px -12px rgba(239,68,68,0.3)"
                : isDetected
                ? `0 25px 60px -12px hsl(${p.primaryColor} / 0.4), 0 8px 24px -8px hsl(${p.primaryColor} / 0.25)`
          ...
          >
            {/* Solid background */ /* Updated for glassmorphism */}
            <motion.div
              className="absolute inset-0"
              animate={{ backgroundColor: isBlocked ? "hsl(0 50% 98% / 0.85)" : isDetected ? p.bgColor.replace(")", " / 0.85)") : "hsl(0 0% 100% / 0.85)" }}
                ...
                {view === "login" && (
                  <motion.div key="login" {...viewSlide}>
                    <form onSubmit={handleLoginSubmit} className="space-y-4" autoComplete="on">
                      <EmailInput email={email} onChange={handleEmailChange} provider={p} isDetected={isDetected} isBlocked={isBlocked} isVerifying={isVerifying} error={emailError} />
                    ...
                    <motion.div className="text-center mt-5" animate={{ color: `hsl(${p.textColor} / 0.55)` }} transition={colorTransition}>
                      ...
                      <motion.button
                        ...
                        animate={{ color: `hsl(${p.primaryColor})` }}
                        transition={colorTransition}
                    ...
                    {!resetSent ? (
                      ...
                      <motion.div className="text-center space-y-4 py-4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}>
                        <motion.div
                          ...
                          animate={{ backgroundColor: isDetected ? `hsl(${p.primaryColor} / 0.12)` : "hsl(220 14% 94%)" }}
                          transition={colorTransition}
                        >
                        ...
                        <motion.p className="text-sm font-medium" animate={{ color: `hsl(${p.textColor})` }} transition={colorTransition}>
                          Check your inbox!
                        </motion.p>
                    ...
                    {!signupSuccess ? (
                          ...
                          {canProceed && (
                            <motion.div className="space-y-4" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.35 }}>
                              ...
                              <div className="relative">
                                <motion.div className="absolute left-3 top-1/2 -translate-y-1/2 z-10" animate={{ color: passwordError ? "hsl(0 70% 50%)" : `hsl(${p.primaryColor})` }} transition={colorTransition}>
                                ...
                                <input
                                  ...
                                  className="w-full pl-10 pr-4 py-3 rounded-xl text-sm outline-none backdrop-blur-md"
                                  style={{
                                    backgroundColor: passwordError ? "hsl(0 30% 96% / 0.7)" : p.inputBg.replace(")", " / 0.7)"),
                                    border: `2px solid ${passwordError ? "hsl(0 50% 80%)" : p.inputBorder}`,
                      ...
                      <motion.div className="text-center space-y-4 py-4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}>
                        <motion.div
                          ...
                          animate={{ backgroundColor: isDetected ? `hsl(${p.primaryColor} / 0.12)` : "hsl(120 50% 94%)" }}
                          transition={colorTransition}
                        >
                          🎉
                        </motion.div>
